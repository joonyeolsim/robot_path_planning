...
class Environment(object):
    ...
    def wait_physical_conflict(self, solution):
        max_t = max([len(plan) for plan in solution.values()])
        for t in range(max_t):
            for agent_1, agent_2 in permutations(solution.keys(), 2):
                state_1a = self.get_state(agent_1, solution, t)
                state_1b = self.get_state(agent_1, solution, t + 1)
                ...
                if state_1b.is_equal_except_time(state_2a):
                    if abs(state_2b.location.y - state_1a.location.y) + abs(state_2b.location.x - state_1a.location.x) == 2:
                        solution[agent_1].insert(t, State(t, Location(state_1a.location.x, state_1a.location.y)))
                        for state in solution[agent_1][t+1:]:
                            state.time += 1
        return solution
...
class CBS(object):
    ...
    def search(self):
        ...
        while self.open_set:
            ...
            P.solution = self.env.wait_physical_conflict(P.solution)
